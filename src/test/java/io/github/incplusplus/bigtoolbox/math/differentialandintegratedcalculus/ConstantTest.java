...
class ConstantTest {
  ...
  void integration() {
    Constant testConstant = new Constant(new BigDecimal(posVal));
    assertEquals(
        testConstant.integration(posVal).compareTo(new BigDecimal(posVal).pow(2)), zeroVal);

    testConstant = new Constant(new BigDecimal(negVal));
    assertEquals(
        testConstant.integration(negVal).compareTo(new BigDecimal(negVal).pow(2)), zeroVal);

    testConstant = new Constant(new BigDecimal(zeroVal));
    assertEquals(
        testConstant.integration(zeroVal).compareTo(new BigDecimal(zeroVal).pow(2)), zeroVal);
  }
