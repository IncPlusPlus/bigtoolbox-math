...
public class AmicableMath {
  /**
   * a and b are an amicable pair if d(a) = b and d(b) = a, where a ≠ b Let d(n) be defined as the
   * sum of proper divisors of n (numbers less than n which divide evenly into n).
   *
   * @param a
   * @param b
  ...
  public static boolean amicablePair(int a, int b) {
    return DivisorMath.sumProperDivisors(a) == b && DivisorMath.sumProperDivisors(b) == a && a != b;
  }
   ...
   * @return a number that makes an amicable pair when paired with a.. If none exists or the number
   *     found is greater than the limit, returns 0
   */
