...
public class MiscUtils {
  ...
  public static boolean isPalindrome(int in) {
    ...
    while (length(numberInQuestion) > 1) {
      if (leftDigit(numberInQuestion) != rightDigit(numberInQuestion)) {
        ...
        if (length(truncateRightDigit(numberInQuestion))
                - length(truncateLeftDigit(numberInQuestion))
            != 0) {
          for (int i = 0;
              i < length(numberInQuestion) - length(truncateLeftDigit(numberInQuestion));
              i++) {
  ...
  public static int distance(BigDecimal x1, BigDecimal y1, BigDecimal x2, BigDecimal y2) {
    return (MiscUtils.sqrt(
            (MiscUtils.pow(x2.subtract(x1), 2)).add(MiscUtils.pow(y2.subtract(y1), 2))))
        .intValue();
  }
  ...
  /**
   * The following functions are wrapper functions. I made them for the purpose of using easy
   * function names such as sin(x) to work with BigDecimal objects. BigDecimal objects were used to
   * lose as little precision as possible. Use the functions below with caution. They are
   * experimental.
   */
